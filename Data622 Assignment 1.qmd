...
time_rf_train = system.time({
  rf_model = train(Class ~ ., data = dataTrain, method = "ranger", trControl = train_control, tuneGrid = rf_grid)})
```
...
time_rf_pred = system.time({
  rf_pred = predict(rf_model, dataTest)
  rf_probs = predict(rf_model, dataTest, type = "prob")[, 2]
})
...
confusionMatrix(rf_pred, dataTest$Class)
```
...
benchmark_results = data.frame(
  ...
  Training_Time = c(time_logistic_train[3], time_tree_train[3], time_rf_train[3]),
  Prediction_Time = c(time_logistic_pred[3], time_tree_pred[3], time_rf_pred[3])
)
...
roc_rf = roc(dataTest$Class, rf_probs)
